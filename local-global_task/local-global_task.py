...
Tha task is to respond to targets (i.e., q for a circle, w for an X,
 a for e triangle, and r for a square). If the figure is blue
...
If the target is black participants are to respond to the local figure. In
...
class Task():
    ...
    def randomizeTrials(self,stimuli, ntrials):
        '''
        Will randomize 50 % of shifting trials
        based on that each filename (of the images) starts with Black or Blue-.
        
        stimuli is a list of trials
        ntrials is an integer indicating number of trials
        '''
        nEachstim = ntrials/len(stimuli)
        nEachtype = ntrials/2
        trialTypes = []
        stims = []
        countOfStim = dict((el,0) for el in stimuli)
        count = {'ns':0,'s':0}
        for i in range(ntrials):
            shuffle(stimuli)
            for idx in range(len(stimuli)):
                if not stims:
                    countOfStim[stimuli[idx]] +=1
                    stims.append(stimuli[idx])
                    count['ns'] +=1
                    trialTypes.append("No-Shifting")
                elif stims:
                    if count['s'] <= nEachtype:
                        if countOfStim[stimuli[idx]] <= nEachstim-1:
                            if stimuli[idx][:5] != stims[i-1][:5]:
                                count['s'] +=1
                                countOfStim[stimuli[idx]] +=1
                                stims.append(stimuli[idx])
        ...
        if freq[0] == freq[1] and sum(freq) == ntrials:
            ...
            return self.randomizeTrials(stimuli, ntrials)
    ...
    def CreateExpTrials(self, trialList, expinfo, files, practice=False):
        ...
        for trial, target in enumerate(trials):#Here i could use enumerate(trial, trialSeq)
            ...
            if parts[0] == "Blue":
                TrialType = 'Global'
                if parts[1] == "Circle":
                    self.correctResp = responseButtons[0]
                elif parts[1] == "X":
                    self.correctResp = responseButtons[1]
                elif parts[1] == "Triangle":
                    self.correctResp = responseButtons[2]
                elif parts[1] == "Rect":
                    self.correctResp = responseButtons[3]
...
def main():
    ...
    info = experimentWindow.expInfo(expName = "Local-Global Task")
    load = fileHandling()
    load.makeDir('Data')
    task = Task(96,40)
    win = experimentWindow.expWin(color="white")
    ...
    task.waitForKeys(win)
    #Practice Trials
    pracList = task.randomizeTrials(keyList, 40)
    pracHand = task.randomizeTrials(pracList, info, files,  practice=True)
